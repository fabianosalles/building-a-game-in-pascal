...
type
  ...
  public
    ...
    property GUILarge : TGameFont read fGUILarge write fGUILarge;
  ...
  public
    constructor Create( const aRenderer: PSDL_Renderer );
    destructor Destroy; override;
    procedure Draw( const aText : string; x, y : integer; aFont : TGameFont ); overload;
    procedure Draw( const aText : string; x, y : integer; aFont : TGameFont; alpha: UInt8 ); overload;
...
begin
  ...
  if fDebugNormal.Font = nil then begin
  ...
  fGUILarge.FileName := aFontsDirectory + 'Arcade.ttf';
  fGUILarge.Size     := 60;
  fGUILarge.Font     := TTF_OpenFont( PAnsiChar(fGUILarge.FileName), fGUILarge.Size);
  fGUILarge.Color.r  := $FF;
  fGUILarge.Color.g  := $FF;
  fGUILarge.Color.b  := $FF;
  fGUILarge.Color.a  := $FF;

  if fGUILarge.Font = nil then
     raise GameFontException.Create( TTF_GetError );
